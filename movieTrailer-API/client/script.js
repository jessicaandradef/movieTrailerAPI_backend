...
function populateFilterTitle() {
  ...
  fetch("http://localhost:7009/api/movies")
...
function handleFilterBtnClick() {
  ...
  const url = `http://localhost:7009/api/movies/?${queryParams}`;
  ...
  fetch(url, { headers })
    .then((response) => response.json())
    .then((data) => {
      const cardContainer = document.querySelector("movie__card");
...
const filterMovie = document.getElementById("filter__btn");
filterMovie.addEventListener("click", handleFilterBtnClick);
...
async function login(email, password) {
  try {
    ...
    const response = await fetch("http://localhost:7009/auth/login", {
      method: "POST",
      headers: {
